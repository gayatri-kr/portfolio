...
- **Hero stars** — `Hero3D.tsx` uses `count` from `usePerformantContext()` (desktop: 5000, mobile: 1500, reduced: 800).
- **Hero rings** — 5 torus meshes in `Hero3D.tsx` `Rings` component. Add/remove `<mesh>` blocks.
- **Skills particles** — `Section3DBg.tsx` uses `particleCount` from context (desktop: 5000, mobile: 800, reduced: 300). Capped at 6000 in the component.
...
- **Theme** — Dark/light toggle with smooth transitions.
